/**
 * Les quatre fenêtres de l'onglet Inspections.
 *
 * Extraites de `InspectionsTab.jsx`, qui atteignait 629 lignes pour cinq
 * composants. L'onglet lui-même — la liste, ses filtres, son chargement — n'en
 * représentait que 155 : tout le reste était de la saisie en modale, sans
 * autre lien avec lui que des props.
 *
 * Les garder ensemble avait un coût précis : toute modification de la liste
 * obligeait à faire défiler quatre formulaires, et une recherche dans le
 * fichier ramenait des résultats de trois écrans différents.
 */
import { useCallback, useEffect, useState } from 'react';
import { useTranslation } from 'react-i18next';
import {
  Camera, Check, ClipboardCheck, ListChecks, Plus, Trash2, UserPlus, Users, X, XCircle,
} from 'lucide-react';
...
import {
  creerInspection, getInspection, modifierInspection,
...
import { ApercuProtege } from '../../../../components/FichierProtege.jsx';

/**
 * Identifiant STABLE d'une ligne de checklist.
 *
 * L'index ne peut pas servir de clé : supprimer une ligne décale toutes les
 * suivantes, et React réutilise alors les nœuds du DOM en croyant qu'ils
 * représentent les mêmes éléments — le focus et la position du curseur sautent
 * sur une autre ligne. Un compteur suffit : ces clés ne vivent que le temps
 * d'une saisie et ne sont jamais persistées.
 */
let compteurLigne = 0;
const cleLigne = () => `ligne-${++compteurLigne}`;

/* ============ Création ============ */
export function InspectionCreateModal({ open, onClose, chantierId, onSaved }) {
   ...
   * L'index ne peut pas servir de clé : supprimer une ligne décale toutes les
   * suivantes, et React réutilise alors les nœuds du DOM en croyant qu'ils
   * représentent les mêmes éléments — le focus et la position du curseur
   * sautent sur une autre ligne. Un identifiant attaché à la LIGNE survit à
   * toutes les suppressions.
   */
  const ligneVierge = () => ({ cle: cleLigne(), libelle: '', coche: false, commentaire: '' });

  const [checklist, setChecklist] = useState([ligneVierge()]);
  ...
  useEffect(() => {
    ...
    setChecklist([ligneVierge()]);
    listerMembresChantier(chantierId).then((d) => setMembres(d.items)).catch((err) => reporter(err, { source: 'InspectionsTab' }));
    listerModeles().then((d) => setModeles(d.items)).catch((err) => reporter(err, { source: 'InspectionsTab' }));
  }, [open, chantierId]);
  ...
  const addLine = () => setChecklist([...checklist, ligneVierge()]);
      ...
      <form onSubmit={submit}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <select className="input" value={form.modeleId} onChange={(e) => {
              ...
              if (id) {
                ...
                if (m?.items?.length) setChecklist((m.items || []).map((it) => ({ cle: cleLigne(), libelle: typeof it === 'string' ? it : it?.libelle || '', coche: false, commentaire: '' })));
              } else {
                setChecklist([ligneVierge()]);
              }
            }}>
        ...
        <div className="field">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            {checklist.map((c, i) => (
              <div key={c.cle} style={{ display: 'flex', gap: 6 }}>
                {/* `aria-label` : le placeholder n'est pas un libellé — il
                    disparaît à la première frappe, et aucune synthèse vocale
                    n'en fait le nom du champ. */}
                <input
                  className="input"
                  aria-label={t('inspections.point', { n: i + 1 })}
                  placeholder={t('inspections.point', { n: i + 1 })}
                  value={c.libelle}
                  onChange={(e) => setLine(i, { libelle: e.target.value })}
                />
                {checklist.length > 1 && (
                  <button
                    type="button"
                    className="btn btn-ghost btn-sm"
                    aria-label={t('inspections.retirerPoint', { n: i + 1 })}
...
/* ============ Détail ============ */
export function InspectionDetailModal({ inspection, onClose, onChanged, canManage }) {
  ...
  const loadDetail = useCallback(async () => {
    ...
    try {
      ...
      if (d.chantierId) listerMembresChantier(d.chantierId).then((r) => setMembres(r.items)).catch((err) => reporter(err, { source: 'InspectionsTab' }));
    } catch (err) {
...
export function ModelesModal({ open, onClose, canManage }) {
  const { t } = useTranslation('chantier');
  const [modeles, setModeles] = useState([]);
  const [loading, setLoading] = useState(false);
...
export function ModeleModal({ modele, onClose, onSaved, canManage }) {
  const { t } = useTranslation('chantier');
      ...
      <div className="field">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          {form.items.map((item, i) => (
            /* Ici les entrées sont des CHAÎNES, sans identifiant propre : la
               clé combine la position et le contenu. Elle reste stable tant
               qu'on tape, et change quand la ligne change réellement de
               contenu — bien plus fidèle que l'index seul, qui fait glisser
               toutes les lignes suivantes à chaque suppression. */
            <div key={`${i}-${item}`} style={{ display: 'flex', gap: 6 }}>
              <input
                className="input"
                aria-label={t('inspections.point', { n: i + 1 })}
                value={item}
                onChange={(e) => setForm({ ...form, items: form.items.map((x, idx) => (idx === i ? e.target.value : x)) })}
                placeholder={t('inspections.point', { n: i + 1 })}
              />
              <button
                type="button"
